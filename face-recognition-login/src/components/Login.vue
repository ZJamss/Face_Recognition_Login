<template>
  <div class="app">
    ...
    <el-button @click="onRegister">登录</el-button>
...
export default {
  ...
  methods: {
    async onRegister() {
      if (this.stu.stuPhoto === "") {
        this.$message.warning("请拍摄人脸照片")
      } else {
        let [data, error] = await api.login(this.stu.stuPhoto)
        if (data) {
          let res = data.data
          res = res.replaceAll("////", "");
          res = JSON.parse(res)
          console.log(res.result.user_list[0])
          console.log(data)
          if (data.code === 0 && res.result.user_list[0].score > 80) {
